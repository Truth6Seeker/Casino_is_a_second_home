...
// Add this helper class at the top of main.cpp
class SessionEndGuard {
  std::shared_ptr<GameSession> session_;
  bool started_;

 public:
  explicit SessionEndGuard(std::shared_ptr<GameSession> session)
      : session_(session), started_(false) {}
...
// Modify all game functions like this:
void playDiceGame(std::shared_ptr<GameSession> session) {
  SessionEndGuard guard(session);
  auto diceGame = std::make_shared<DiceGame>();
  if (!session->startGame(diceGame)) {
  ...
  while (true) {
    ...
    std::cout << "New balance: $"
              << casinoRound(session->getPlayer()->getBalance()) << "\n\n";
  }
}
...
void playBlackjack(std::shared_ptr<GameSession> session) {
  ...
  if (!session->startGame(blackjack)) {
    std::cout << "Cannot start game. Another game is active.\n";
    return;
  }
  guard.setStarted();
  std::cout << "\n=== Welcome to Blackjack! ===\n";
  std::cout << "Rules:\n- Try to get 21 or closer than dealer\n";
  std::cout << "- Face cards = 10, Ace = 1 or 11 (auto)\n";
  std::cout << "- Blackjack pays 3:2\n";
  std::cout << "- Win pays 1:1\n\n";
    ...
    while (true) {
      ...
      if (std::cin >> betAmount) {
        ...
        if (betAmount > 0 && betAmount <= balance) {
          if (blackjack->placeBet(betAmount)) {
            break;
          } else {
  ...
  while (true) {
    ...
    while (!validInput) {
      ...
      if (input == "all") {
        ...
        validInput = true;
        std::cout << "All-in bet placed! $" << casinoRound(balance)
                  << " on the line!\n";
      } else {
        try {
...
void playRussianRoulette(std::shared_ptr<GameSession> session) {
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (std::cin >> betAmount) {
        ...
        if (betAmount > 0 && betAmount <= balance) {
          if (rrGame->placeBet(betAmount)) {
            std::cout << "House matches your bet! Total pot: $" << std::fixed
                      << std::setprecision(2) << betAmount * 2 << "\n";
            break;
          } else {
            std::cout << "Failed to place bet. Please try again.\n";
          }
      ...
      std::cout << "Invalid amount! Please enter between 0 and " << balance
                << "\n";
      clearInputBuffer();
    }
    ...
    while (rrGame->isGameActive() && !rrGame->isGameOver()) {
      ...
      std::cout << "  Bullets: " << rrGame->getBulletsRemaining()
                << " remain\n";
      ...
      if (rrGame->isPlayerTurn()) {
        ...
        while (!validAction) {
          ...
          if (action == 'E') {
            ...
            std::cout << "Enter escape amount (min $" << std::fixed
                      << std::setprecision(2) << minEscape << "): $";
            ...
            if (escapeAmount < minEscape) {
              std::cout << "Escape amount must be at least $" << minEscape
                        << "!\n";
              continue;
            }
          }
    ...
    if (rrGame->didPlayerSurvive()) {
      ...
      if (rrGame->isPlayerTurn()) {
        ...
        // Calculate net profit (gross winnings minus original bet)
        double netProfit =
            (rrGame->getCurrentMultiplier() * betAmount * 2) - betAmount;
        std::cout << "You won $" << std::fixed << std::setprecision(2)
                  << netProfit << "!\n";
      } else {

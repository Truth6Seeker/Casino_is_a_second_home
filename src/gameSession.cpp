...
GameSession::GameSession(std::shared_ptr<Player> player)
    : player(player), currentGame(nullptr), startTime(std::time(nullptr)),
      initialBalance(player->getBalance()), totalWagered(0), totalWon(0) {}
...
std::shared_ptr<IGame> GameSession::getCurrentGame() const { return currentGame; }

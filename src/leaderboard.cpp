...
void Leaderboard::addOrUpdatePlayer(const std::string& name, double balance,
  ...
  const char* select_sql =
      "SELECT balance, games_played, wins FROM leaderboard WHERE name = ?;";
  sqlite3_stmt* stmt;
  ...
  std::cout << "[DEBUG] Save to leaderboard: " << name << " | $"
            << final_balance << " | Games: " << final_games
            << " | Wins: " << final_wins << std::endl;
...
std::vector<LeaderboardEntry> Leaderboard::getTopPlayers(int limit) {
  ...
  const char* sql =
      "SELECT name, balance, games_played, wins FROM leaderboard ORDER BY "
      "balance DESC LIMIT ?;";
  sqlite3_stmt* stmt;
  sqlite3_prepare_v2((sqlite3*)db, sql, -1, &stmt, nullptr);

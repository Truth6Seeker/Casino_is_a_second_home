...
std::shared_ptr<Player> Casino::registerPlayer(const std::string &name,
                                               uint32_t age,
...
void Casino::adjustHouseBalance(double amount) { houseBalance += amount; }

double Casino::getHouseBalance() const { return houseBalance; }

void Casino::displayActiveSessions() const {
  std::cout << "Active Sessions in " << name << ":\n";
  for (const auto &pair : activeSessions) {
    auto session = pair.second;
    std::cout << "Player: " << session->getPlayer()->getName() << " | Game: "
              << (session->getCurrentGame()
                      ? session->getCurrentGame()->getGameName()
                      : "None")
